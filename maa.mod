...
set Av := {v in V, (v,i) in NV, (v,j) in NV: j <> home_port[v]};
...
# display {v in V, c in C, i in {port[c]},npi in {port[c+cargoes]}: 
#                 can_transport[v, c]
#         } (v,c,i,npi);
